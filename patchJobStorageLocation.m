...
% clashes that would otherwise arise. 
%
% NOTES:
% 1) You will need to set the JobStorageLocation of any subsequent MATLAB by hand if
%    you want to retrieve the results of a job in a different MATLAB session. 
% 2) If you want to make "JobStorageLocation"s unique across multiple computers (rather 
%    than just within one computer) then it would be best to change the uniqueEndingStr 
%    below to call 'tempname' and extract the final part. That will ensure that even 
%    with PID reuse across multiple machines you still get a unique string for each MATLAB.

%   Copyright 2016 The MathWorks, Inc.
%   https://www.mathworks.com/matlabcentral/answers/545174

try    
    % Check to see if PCT (Parallel Computing Toolbox) is installed. If not simply return
    % early as there is nothing to do.
    ...
    for index = 1:numel(profiles)
        ...
        % level so that when we restart the system it goes back to the previous
        % setting
